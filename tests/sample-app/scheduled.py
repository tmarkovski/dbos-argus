...
def heartbeat_check(scheduled_at: datetime, context: Any = None) -> None:
    """Tick once a minute. Each run lands a workflow row in the dashboard.
...
def register_schedules() -> None:
    """Idempotent — register the heartbeat schedule, no-op if it already exists."""
    existing = DBOS.list_schedules(schedule_name_prefix=HEARTBEAT_SCHEDULE_NAME)
    if any(s["schedule_name"] == HEARTBEAT_SCHEDULE_NAME for s in existing):
        return
